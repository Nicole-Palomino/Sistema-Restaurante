...
<jsp:useBean id="DatosDetalle" class="model.BDetallePedido" scope="request"></jsp:useBean>
<body>
	...
	<div class="container mt-3">
		<div class="border border-4 w-50 mx-auto rounded-4 border-dark p-2 mb-2 border-opacity-50">
			...
			<form action="SActualizarDetalle" method="post" class="w-75 mx-auto needs-validation">
			<div class="input-group flex-nowrap mt-4">
					<span class="input-group-text" id="addon-wrapping">
						<i class="bi bi-basket3-fill"></i>
					</span>
					<input type="text" class="form-control" name="txtPed" value="<%=DatosDetalle.getIdpedido()%>" readonly>
				</div>
				<div class="input-group flex-nowrap mt-4">
					<span class="input-group-text" id="addon-wrapping">
						<i class="bi bi-basket-fill"></i>
					</span>
					<input type="text" class="form-control" name="txtProd" value="<%=DatosDetalle.getIdproducto()%>" readonly>
				</div>
				<div class="input-group flex-nowrap mt-4">
					<span class="input-group-text" id="addon-wrapping">
						<i class="bi bi-list-ol"></i>
					</span>
					<input type="text" class="form-control" name="txtCant" value="<%=DatosDetalle.getCantidad() %>" readonly>
				</div>
				<div class="input-group flex-nowrap mt-4">
					<span class="input-group-text" id="addon-wrapping">
						<i class="bi bi-question-circle-fill"></i>
					</span>
					<input type="text" class="form-control" name="txtEst" value="<%=DatosDetalle.getEstado() %>">
				</div>
				<div class="form-group mt-2 mb-3">
				    <button type="submit" class="btn btn-success mt-2 w-100">ACTUALIZAR</button>
				    <a class="btn btn-danger mt-2 w-100" href="SListaDetalle">CANCELAR</a>
				</div>
				
				<input type="hidden" name="codigo" value="<%= DatosDetalle.getCod_detalle()%>">
			</form>
		</div>
	</div>
